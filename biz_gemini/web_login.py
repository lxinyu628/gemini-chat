...
class LoginStatus(str, Enum):
    """登录状态枚举"""
    IDLE = "idle"  # 空闲
    STARTING = "starting"  # 正在启动浏览器
    WAITING = "waiting"  # 等待用户登录
    PROCESSING = "processing"  # 处理登录信息
...
class LoginTask:
    ...
    def update(self, status: LoginStatus, message: str = "", progress: int = None):
        """更新任务状态"""
        self.status = status
...
class WebLoginService:
    ...
    async def _run_login(self, task: LoginTask):
        """执行登录流程（后台任务）"""
        try:
            ...
            # 调用浏览器登录（这是一个异步函数）
            # 注意：原始的 login_via_browser 可能需要修改以支持 headless 模式
