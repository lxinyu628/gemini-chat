    ...
    async def _do_refresh(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                if new_config:
                    # 如果 _extract_cookies 没有从 URL 获取到 csesidx，保持使用原来的值
                    if not new_config.get("csesidx"):
                        old_csesidx = config.get("csesidx")
                        if old_csesidx:
                            new_config["csesidx"] = old_csesidx
                            logger.debug(f"[浏览器保活] 保持原 csesidx: {old_csesidx}")
